...
int handle_client(int client_fd, struct sockaddr_in client_addr, std::string dir) {
  char buff[2048];
  ...
  if (request.method == "GET") {
    if (request.path == "/") {
      ...
      response = "HTTP/1.1 200 OK\r\nContent-Type: application/octet-stream\r\nContent-Length: " + std::to_string(body.str().length()) + "\r\n\r\n" + body.str();
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    std::thread th(handle_client, client_fd, client_addr, dir);
    th.detach();
  }
